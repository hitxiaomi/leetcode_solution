...
class Solution {
 public:
  int maxProfit(vector<int>& prices) {
    int max_profit = 0;
    bool own_stocks = false;
    int own_stocke_intial_price = 100000;
    size_t prices_size = prices.size();

    for (int i = 0; i < prices_size - 1; i++) {
      if ( !own_stocks && prices[i] < prices[i + 1]) {
        own_stocke_intial_price = prices[i];
        own_stocks = true;
      } else if ( own_stocks && prices[i] > prices[i + 1]) {
        max_profit += prices[i] - own_stocke_intial_price;
        own_stocks = false;
      }
    }
    if (own_stocks) {
      max_profit += prices[prices_size - 1] - own_stocke_intial_price;
    }
    return max_profit;
  }
};
