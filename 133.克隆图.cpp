...
class Solution {
private:  
    unordered_map<Node* ,Node*> map_node_node;//存储原图Node和当前图之间的映射
public:
    Node* cloneGraph(Node* node) {
        //两个基线条件
        if(!node) return nullptr;//空节点 //
        if(map_node_node.find(node)!=map_node_node.end()){ //已经存在该节点
            return  map_node_node[node];
        }

        Node* result_node=new Node();
        result_node->val=node->val;
        map_node_node[node]=result_node;//添加到map_node_node中

        for(auto const & neighbor:node->neighbors){
            if(neighbor) {
                result_node->neighbors.push_back(cloneGraph(neighbor));
            }
